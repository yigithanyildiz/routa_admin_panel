...
import { getDestination, updateDestination } from '@/lib/firebase-service';
...
export default function EditDestinationPage() {
    ...
    const loadDestination = async () => {
        try {
            ...
            if (!dest) {
                toast.error('Destinasyon bulunamadı');
            ...
            setFormData({
                ...
                latitude: dest.coordinates.latitude.toString(),
                longitude: dest.coordinates.longitude.toString(),
                summerTemp: dest.averageTemperature.summer.toString(),
                winterTemp: dest.averageTemperature.winter.toString(),
                popularMonths: dest.popularMonths || [],
                costLevel: dest.costOfLiving?.level || '',
                dailyBudgetMin: dest.costOfLiving?.dailyBudgetMin?.toString() || '',
                dailyBudgetMax: dest.costOfLiving?.dailyBudgetMax?.toString() || '',
                travelStyle: dest.travelStyle || [],
                bestFor: dest.bestFor || [],
                rating: dest.rating?.toString() || '',
                popularity: dest.popularity?.toString() || '',
            });
        } catch (error) {
            console.error('Error loading destination:', error);
            toast.error('Destinasyon yüklenirken hata oluştu');
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            toast.success('Destinasyon başarıyla güncellendi');
            router.push('/dashboard/destinations');
        } catch (error) {
            console.error('Error updating destination:', error);
            toast.error('Destinasyon güncellenirken hata oluştu');
        } finally {
            setSaving(false);
        }
    };
        ...
        <div className="p-6 space-y-6 max-w-4xl">
            ...
            <div className="flex items-center gap-4">
                ...
                <div className="space-y-1">
                    <h1 className="text-3xl font-bold text-white">Destinasyon Düzenle</h1>
